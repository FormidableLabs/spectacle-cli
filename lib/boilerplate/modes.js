 ...
 * - `templates/{MODE}-slides/index.js`: We use the same regex replacement
...
const { modes } = require('../templates/replacements');
...
const baseGenerate = async ({ name, description, dir, files }) => {
  ...
  const allFiles = {
    ...
    // Extra boilerplate templates.
    // **NOTE**: `_`-prefixed files like `_gitignore` are named specially so
    // that this project CLI can keep the files when published to npm.
...
const js = async ({ name, description, dir }) => {
  const files = {
    '../templates/js-slides/index.html': 'src/index.html',
    '../templates/js-slides/index.js': 'src/index.js'
  };
