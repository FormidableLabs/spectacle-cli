...
const webpackConfig = async ({
  ...
  const platformSafePath = srcFilePath.replace(singleBackslashRegex, "\\\\");

  // Templates, examples from dependencies.
  const tmplDir = getTmplDir(srcMode);
  const platformSafeThemePath = themeFilePath && themeFilePath.replace(singleBackslashRegex, "\\\\");
  const platformSafeTemplatePath = templateFilePath && templateFilePath.replace(singleBackslashRegex, "\\\\");
                ...
                replacements: [
                  {
                    ...
                    replacement: modes[srcMode].tmpl(platformSafePath)
                  },
                  ...
                  // is originally in the example.
                  {
                    ...
                    replacement: templateFilePath
                      ? templateTmpl(platformSafeTemplatePath)
                      : templateDefault()
                  },
                  {
                    pattern: themeRe,
                    replacement: themeFilePath
                      ? themeTmpl(platformSafeThemePath)
                      : themeDefault()
                  }
                ].filter(Boolean)
              }
